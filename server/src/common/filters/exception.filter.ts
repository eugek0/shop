...
export class AllExceptionFilter implements ExceptionFilter {
  ...
  private loggingByExceptionInfo(
    ...
    const { message, statusCode, error } = exceptionInfo;
    const { path, ip } = req;
    const email = req?.['user']?.email;
    const id = req?.['user']?.id;

    const transformMessage = Array.isArray(message)
      ? message.join('\n   Message: ')
      : message;
  ...
  get defaultLanguage(): string {
    return this.configService.get('ERROR_LANGUAGE');
  }
  ...
  get logger(): Logger {
    return new Logger(AllExceptionFilter.name);
  }
